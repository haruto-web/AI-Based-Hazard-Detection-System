import { $s as expandDims, Nl as concat2d, Qt as fromPixels, Xd as tidy, _u as concat, cd as mul, fu as slice, ii as tensor1d, kn as image, ld as div, mi as squeeze, mu as sigmoid, n as init_dist, ni as tensor2d, p as init_dist$1, pd as add, rs as sub, s as loadGraphModel, wu as reshape, xf as Tensor, yd as cast } from "./dist-C4JVxQf8.js";
//#region node_modules/@tensorflow-models/blazeface/dist/blazeface.esm.js
init_dist();
init_dist$1();
/**
* @license
...
var scaleBox = function(box, factors) {
	return createBox(concat2d([mul(box.startPoint, factors), mul(box.endPoint, factors)], 1));
};
/**
* @license
* Copyright 2019 Google LLC. All Rights Reserved.
...
function generateAnchors(width, height, outputSpec) {
	...
	for (var i = 0; i < outputSpec.strides.length; i++) {
		...
		for (var gridY = 0; gridY < gridRows; gridY++) {
			var anchorY = stride * (gridY + .5);
			for (var gridX = 0; gridX < gridCols; gridX++) {
				var anchorX = stride * (gridX + .5);
				for (var n = 0; n < anchorsNum; n++) anchors.push([anchorX, anchorY]);
			}
		}
	}
	return anchors;
...
function decodeBounds(boxOutputs, anchors, inputSize) {
	var centers = add(slice(boxOutputs, [0, 1], [-1, 2]), anchors);
	var boxSizesNormalized = div(slice(boxOutputs, [0, 3], [-1, 2]), inputSize);
	...
	return concat2d([mul(starts, inputSize), mul(ends, inputSize)], 1);
}
...
function flipFaceHorizontal(face, imageWidth) {
	...
	if (face.topLeft instanceof Tensor && face.bottomRight instanceof Tensor) {
		var _a = tidy(function() {
			return [concat([slice(sub(imageWidth - 1, face.topLeft), 0, 1), slice(face.topLeft, 1, 1)]), concat([sub(imageWidth - 1, slice(face.bottomRight, 0, 1)), slice(face.bottomRight, 1, 1)])];
		}), topLeft = _a[0], bottomRight = _a[1];
		flippedTopLeft = topLeft;
		flippedBottomRight = bottomRight;
		if (face.landmarks != null) flippedLandmarks = tidy(function() {
			return mul(sub(tensor1d([imageWidth - 1, 0]), face.landmarks), tensor1d([1, -1]));
		});
	} else {
		...
		if (face.landmarks != null) flippedLandmarks = face.landmarks.map(function(coord) {
			return [imageWidth - 1 - coord[0], coord[1]];
		});
	}
	var flippedFace = {
	...
	if (flippedLandmarks != null) flippedFace.landmarks = flippedLandmarks;
	if (face.probability != null) flippedFace.probability = face.probability instanceof Tensor ? face.probability.clone() : face.probability;
	return flippedFace;
}
function scaleBoxFromPrediction(face, scaleFactor) {
	return tidy(function() {
		var box;
		if (face.hasOwnProperty("box")) box = face.box;
		else box = face;
		...
		return __awaiter(this, void 0, void 0, function() {
			...
			return __generator(this, function(_b) {
				switch (_b.label) {
					case 0:
						_a = tidy(function() {
							var normalizedImage = mul(sub(div(image.resizeBilinear(inputImage, [_this.width, _this.height]), 255), .5), 2);
							var prediction = squeeze(_this.blazeFaceModel.predict(normalizedImage));
							return [
								prediction,
								decodeBounds(prediction, _this.anchors, _this.inputSize),
								squeeze(sigmoid(slice(prediction, [0, 0], [-1, 1])))
							];
					...
					case 3:
						...
						if (returnTensors) scaleFactor = div([originalWidth, originalHeight], this.inputSize);
						else scaleFactor = [originalWidth / this.inputSizeData[0], originalHeight / this.inputSizeData[1]];
						annotatedBoxes = [];
						_loop_1 = function(i) {
							...
							var annotatedBox = tidy(function() {
								...
								if (!annotateBoxes) return box;
								var boxIndex = boxIndices[i];
								var anchor;
								if (returnTensors) anchor = slice(_this.anchors, [boxIndex, 0], [1, 2]);
								else anchor = _this.anchorsData[boxIndex];
								return {
									box,
									landmarks: reshape(squeeze(slice(detectedOutputs, [boxIndex, NUM_LANDMARKS - 1], [1, -1])), [NUM_LANDMARKS, -1]),
									probability: slice(scores, [boxIndex], [1]),
									anchor
								};
							});
							annotatedBoxes.push(annotatedBox);
						};
						for (i = 0; i < boundingBoxes.length; i++) _loop_1(i);
						boxes.dispose();
						scores.dispose();
			...
			return __generator(this, function(_c) {
				switch (_c.label) {
					case 0:
						...
						image = tidy(function() {
							if (!(input instanceof Tensor)) input = fromPixels(input);
					...
					case 1:
						...
						image.dispose();
						if (returnTensors) return [2, boxes.map(function(face) {
							...
							if (flipHorizontal) normalizedFace = flipFaceHorizontal(normalizedFace, width);
									...
									switch (_c.label) {
										...
										case 3:
											...
											scaledLandmarks = landmarkData.map(function(landmark) {
												return [(landmark[0] + anchor_1[0]) * scaleFactorX_1, (landmark[1] + anchor_1[1]) * scaleFactorY_1];
											});
											normalizedFace = {
												topLeft: boxData.slice(0, 2),
												bottomRight: boxData.slice(2),
...
*/
function load(_a) {
	var _b = _a === void 0 ? {} : _a, _c = _b.maxFaces, maxFaces = _c === void 0 ? 10 : _c, _d = _b.inputWidth, inputWidth = _d === void 0 ? 128 : _d, _e = _b.inputHeight, inputHeight = _e === void 0 ? 128 : _e, _f = _b.iouThreshold, iouThreshold = _f === void 0 ? .3 : _f, _g = _b.scoreThreshold, scoreThreshold = _g === void 0 ? .75 : _g, modelUrl = _b.modelUrl;
	return __awaiter(this, void 0, void 0, function() {
		var blazeface, model;
		return __generator(this, function(_h) {
			switch (_h.label) {
				...
				case 1:
					blazeface = _h.sent();
					return [3, 4];
				case 2: return [4, loadGraphModel(BLAZEFACE_MODEL_URL, { fromTFHub: true })];
				case 3:
					blazeface = _h.sent();
					_h.label = 4;
				case 4:
